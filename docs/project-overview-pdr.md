...
### 功能增強
- **TWSE假期自動化**: 動態從官方API獲取交易日期，無需手動維護
- **as_of_date參數**: 三個評分器和引擎支援歷史日期評分，啟用 backtest 功能
- **AI分析全面化**: LLM分析不再限制Top 10，對所有評分股票進行分析
- **Backtest改進**: 新增 stock_ids 參數支援特定股票績效計算
- **Pipeline最佳化**: 非交易日略過執行，無需建立pipeline_log

### 測試擴展
- 新增 `test_analysis_steps.py` (115行, 7個測試)
- 總測試規模達 140+個，覆蓋 2004 行測試代碼
- 所有新功能都有完整的單元測試驗證

### 依賴更新
- bcrypt: 4.1.1 → 4.2.0
- 新增 requests 庫用於 TWSE API
- 所有核心依賴升至穩定最新版本

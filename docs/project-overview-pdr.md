...
- **後端新增**：`RightSideSignalDetector` 新增 4 個方法
  ...
  - `_calc_risk_level()`: 風險評估（依波動率+分數判定 low/medium/high）
  - `_check_strong_recommend()`: 強力推薦（分數≥60, 觸發≥3, 週趨勢向上, 非高風險）
...
- **後端實現**：`RightSideSignalDetector` 檢測 6 個動能進場信號（需 ≥20 天資料）
  ...
  - 批量掃描範圍：Top 100 評分股 + 近7日量 > 200 萬股聯集
- **前端實現**：
  - 獨立篩選頁面（`/right-side`）：分頁、依加權評分排序、最少信號數過濾（1-6）、4個篩選 toggle

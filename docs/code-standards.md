...
實施於 Dashboard、篩選結果表、右側買法篩選、歷史回測等所有排名表格：

```
分數 ≥ 69.95   →   綠色 (Strong Buy)
分數 ≥ 49.95   →   黃色 (Hold)
分數 < 49.95   →   紅色 (Avoid)
```

**實現位置**
- 後端：`scoring_engine.py` 存儲評分至 `score_result` 表
- 前端：`factor-score-card.vue` 使用 `toFixed(1)` 顯示小數（如 69.9、49.8）
- 所有表格採統一 CSS class（`.score-green`, `.score-yellow`, `.score-red`）

### 儲存精度

- 資料庫儲存：FLOAT（支援 0-100.00）
- API 返回：保留 1 位小數（如 75.4、68.9）
- 前端顯示：保留 1 位小數；門檻判斷使用 ≥69.95 / ≥49.95（容許浮點誤差）
